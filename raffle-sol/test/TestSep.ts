...
describe.only("RaffleSepolia", function () {
  async function deployOneYearLockFixture() {
    ...
    const swapRouterAddress: address = "0xC532a74256D3Db42D0Bf7a0400fEFDbad7694008"; //* Uniswap SwapRouter 
    ...
    // const tokenAddress: address = "0xf08A50178dfcDe18524640EA6618a1f965821715"; //* USDC sepolia token address
    const tokenAddress: address = "0x94a9D9AC8a22534E3FaCa9F4e7F2E2cf85d5E4C8"; //?GK USDC sepolia token address
    ...
    const ethAddress: address = "0x7b79995e5f793a07bc00c21412e50ecae098e7f9"; //? WETH sepolia token address
    // const ethAddress: address = "0x7b79995e5f793A07Bc00c21412e50Ecae098E7f9"; //* WETH sepolia token address
  ...
  describe("Depping", function () {
    it("Should dep and win (normal flow)", async function () {
      const { publicClient, testClient, VRFCoordinatorMock, otherAccount, owner, RaffleContract, swapRouterContract, ethAddress, ethTokenContract, ownerAddress, raffleAddress, tokenContract, tokenAddress } = await loadFixture(deployOneYearLockFixture)
      ...
      await RaffleContract.write.addToWhiteList([ethAddress, BigInt (8), "0x694AA1769357215DE4FAC081bf1f309aDC325306"])
      ...
      //* guy deposit
      try {
        await expect( RaffleContract.write.dep([ethAddress, amount])).to.be.not.rejected;
      } catch (error) {
        console.log(error);
      }
      ...
      // * no balance 
      // expect(Number(await ethTokenContract.read.balanceOf([raffleAddress]))).to.be.equal(0);
    ...
    it("Should dep and win (keeper flow)", async function () {
      const { publicClient, testClient, VRFCoordinatorMock, tokenContract, owner, RaffleContract, swapRouterContract, ethAddress, ethTokenContract, ownerAddress, raffleAddress } = await loadFixture(deployOneYearLockFixture)
    ...
    it("Should dep and not fail due to small dep amount", async function () {
      const { publicClient, testClient, VRFCoordinatorMock, otherAccount, owner, RaffleContract, swapRouterContract, ethAddress, ethTokenContract, ownerAddress, raffleAddress } = await loadFixture(deployOneYearLockFixture)
  ...
  describe("Automation", function () {
    it("Check upkeep", async function () {
      ...
      expect(await RaffleContract.read.checkUpkeep([ethTokenContract.address])).to.be.deep.equal([false, '0x'])

      await time.increase(100);

      expect(await RaffleContract.read.checkUpkeep([ethTokenContract.address])).to.be.deep.equal([true, '0x'])
    })
  })
